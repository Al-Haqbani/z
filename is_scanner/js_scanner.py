...
_REPO_RE = re.compile(r"https?://(?:github\.com|gitlab\.com|bitbucket\.org)/[A-Za-z0-9._-]+/[A-Za-z0-9._-]+")
_PATH_RE = re.compile(r"['\"](/[^'\"\s]{3,})['\"]")


def _check_path(base: str, path: str) -> tuple:
    """Return GET and POST status codes for ``base`` + ``path``."""
...
def _extract_extras(text: str):
    """Return lists of hidden paths and repository links in text."""
    repos = set(_REPO_RE.findall(text))
    paths = set(_PATH_RE.findall(text))
    return list(paths), list(repos)
...
def run_smart_scan(
    ...
    *,
    ...
    while idx < len(js_urls):
        ...
        paths, repos = _extract_extras(text)
        for p in paths:
            g, pstat = _check_path(base, p)
            results.append({"url": url, "leak_type": "hidden-path", "value": p,
                           "get": g, "post": pstat})
        for r in repos:
            results.append({"url": url, "leak_type": "repo-link", "value": r})
...
def run_file_scan(path: str, *, use_linkfinder: bool = False) -> List[Dict[str, str]]:
    ...
    while idx < len(js_urls):
        ...
        for leak_type, value in detect_leaks(text):
            results.append({"url": url, "leak_type": leak_type, "value": value})
        parts = urlparse(url)
        base = f"{parts.scheme}://{parts.netloc}" if parts.scheme else ""
        paths, repos = _extract_extras(text)
        for p in paths:
            g, pstat = _check_path(base, p) if base else (None, None)
            results.append({"url": url, "leak_type": "hidden-path", "value": p,
                           "get": g, "post": pstat})
        for r in repos:
            results.append({"url": url, "leak_type": "repo-link", "value": r})
