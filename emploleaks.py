...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=int(os.getenv("EMPLOLEAKS_THREADS", 0)),
    )
    parser.add_argument(
        "--platforms",
...
def _create_cli_scan(keyword):
    ...
    record_scan(scan_id, keyword, start)
    return scan_id, q
...
def _finalize_cli_scan(scan_id, results):
    ...
    if web_app:
        ...
        SCAN_HISTORY[scan_id]["finished"] = finished
        SCAN_QUEUES.get(scan_id, queue.Queue()).put(None)


def start_web_ui(host="127.0.0.1"):
    ...
    def _run():
        web_app.run(port=8000, host=host)
...
def main():
    ...
    short_token = get_token("Shorteners", "SHORTURL_TOKEN", config.get("shorturl_token"))
    ...
    if args.wordlist and args.keyword:
        ...
        elif args.org and github_token:
            repos = GitHubSearcher.get_org_repos(args.org, github_token)
        path = build_wordlist(args.keyword, repos=repos)
    ...
    while True:
        ...
        if choice == "1":
            ...
            results = SearchManager.start_search(
                ...
                full_scan=full_repo,
                repo=repo,
                domains=domains if platform == "shorturl" else None,
                company=company if platform == "shorturl" else None,
            ...
            if results:
                ...
                csv_path = save_csv_report(results, path=f"reports/{scan_id}.csv")
                print(f"Report saved to {report_path}, {json_path} and {csv_path}")
            else:
            ...
            start_web_ui(args.host)
            from is_scanner.js_scanner import run_file_scan

            file_path = input("Path to file with JS URLs: ").strip()
            use_lf = input("Run LinkFinder? (y/N): ").lower() == "y"
            scan_id, q = _create_cli_scan(file_path)
            found = run_file_scan(file_path, use_linkfinder=use_lf)
            results = []
            for item in found:
                res = {
                    "source": "JavaScript",
            ...
            if repo:
                repos = [repo]
            elif input("Enumerate org repos? (y/N): ").lower() == "y":
                org = input("Organization name: ").strip()
                repos = GitHubSearcher.get_org_repos(org, github_token) if org else []
            from utils.wordlist_builder import build_wordlist
