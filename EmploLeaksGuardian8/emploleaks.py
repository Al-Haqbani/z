...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            users = input("Comma-separated GitHub usernames: ").split(',')
            keyword = input("Keyword: ")
            usernames = [u.strip() for u in users if u.strip()]
            results = SearchManager.github_employee_scan(usernames, keyword, token=token)
            if results:
                print_results(results)
            else:
                print("No leaks found.")
