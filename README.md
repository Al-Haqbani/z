...
There is also a Short URL scanner that queries GrayHatWarfare's shorteners API and matches expanded links against your company name or domains.  When available the scanner records the **creation time** and **size** of each short link so you can gauge how old it is and whether it hosts a large payload.
...
Okta API tokens and Azure client secrets are included too, expanding coverage of enterprise services. Trello and Dropbox API tokens have also been added to the detection rules.
...
The web dashboard adopts a dark gradient style with a purple accent and animated cards for a sleek look. Rows fade in as leaks appear for a smoother experience.
