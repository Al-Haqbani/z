...
<html lang=\"en\">
  <head>
    ...
    <link id="theme" href="https://cdn.jsdelivr.net/npm/bootswatch@5.3.2/dist/vapor/bootstrap.min.css" rel=\"stylesheet\">
    ...
    <style>
      ...
      .scan-card { box-shadow: 0 0.5rem 1rem rgba(0,0,0,0.2); cursor: pointer; transition: transform .2s, box-shadow .2s; opacity:0; animation: cardFade .8s forwards; }
      .scan-card:hover { transform: scale(1.03); box-shadow: 0 0.75rem 1.5rem rgba(0,0,0,0.4); }
      @keyframes cardFade { from {opacity:0;} to {opacity:1;} }
      .logo-icon {animation:pulse 2.5s infinite;}
      @keyframes pulse {0%{transform:scale(1);}50%{transform:scale(1.1);}100%{transform:scale(1);}}
      .progress-bar {
        background-image: linear-gradient(90deg,#7F3FBF,#e67e22);
        border-radius:4px;
        color:#fff;
    ...
    <script>
      ...
      function toggleTheme(){const el=document.getElementById("theme");const dark="https://cdn.jsdelivr.net/npm/bootswatch@5.3.2/dist/vapor/bootstrap.min.css";const light="https://cdn.jsdelivr.net/npm/bootswatch@5.3.2/dist/litera/bootstrap.min.css";el.href=el.href.includes("vapor")?light:dark;localStorage.setItem("theme",el.href);}
    </script>
  </head>
    ...
    <nav class=\"navbar navbar-expand-lg navbar-dark bg-primary fixed-top\">
      <div class=\"container\">
        <a class=\"navbar-brand fw-bold\" href=\"/\">
          <i class=\"fa-solid fa-shield-halved me-2 logo-icon\"></i>
          EmploLeaksGuardian
        </a>
...
<button class="btn btn-sm btn-secondary ms-2" onclick="toggleTheme()">Toggle Theme</button>
    ...
    <div class=\"container\">
      <div class=\"text-center mb-4\">
        <i class=\"fa-solid fa-shield-halved fa-3x mb-2 logo-icon\"></i>
...
<button class="btn btn-sm btn-secondary ms-2" onclick="toggleTheme()">Toggle Theme</button>
    ...
    <div class=\"container\">
      ...
      <a href="/download/{{ scan_id }}.html" class=\"btn btn-outline-light mt-3 me-2\">HTML</a>
      <a href="/download/{{ scan_id }}.json" class=\"btn btn-outline-light mt-3 me-2\">JSON</a>
      <a href="/download/{{ scan_id }}.pdf" class=\"btn btn-outline-light mt-3 me-2\">PDF</a>
      ...
      <div class=\"row\">
        <div class=\"col-md-3 sidebar\">
          ...
          <div class=\"progress mb-3\">
            <div id=\"fileBar\" class=\"progress-bar bg-info\" role=\"progressbar\" style=\"width:0%\">0 files</div>
          </div>
    ...
    <script>
      ...
      evt.addEventListener('progress',ev=>{
        ...
        if(info.total){
          ...
          progBar.textContent=`${pct}% - ${info.repo}`;
          document.title = `${pct}% - ${info.repo}`;
        }
        if(info.leaks){
        ...
        if(info.files){
          ...
          fb.style.width='100%';
          fb.textContent=`${info.files} files`;
        }
        if(info.employees){
          document.getElementById('empCount').innerText = info.employees;
        }
      });
      evt.addEventListener('done',()=>{document.getElementById('done').style.display='block'; document.title='Scan complete'; evt.close();});
    </script>
    <footer class="text-center mt-5">
      <small>تم صناعة هذه الأداة بحب عبر shakbany</small>
    </footer>
