...
<html lang=\"en\">
  ...
  <body class=\"bg-dark text-light\">
    ...
    <script>
      ...
      const network = new vis.Network(container, {nodes:nodes, edges:edges}, {physics:{stabilization:false}});
      ...
      function addRepo(repo){
        if(!nodes.get(repo)){
          nodes.add({id:repo,label:repo,color:'#198754',shape:'box'});
          edges.add({from:'root',to:repo});
        }
      }
      evt.addEventListener('message', ev=>{
